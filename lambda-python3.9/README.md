...
lambda-python3.9$ AWS_SAM_STACK_NAME="lambda-python3.9" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "lambda-python3.9"
```
...
## use python package on aws lambda

- レイヤーを追加のディレクトリ構成が間違っていたため、import error が発生
- 解決方法: package 用の zip ファイルの中に python ディレクトリを作成し、その中に各 package がインストールされている状態にする
- https://soypocket.com/it/aws-lambda-python-error-no-module-named/
